...
static void widget_fontbutton_input_by_command(variable *var, char *command);
...
void widget_fontbutton_refresh(variable *var)
{
	...
	while (act) {
		if (input_is_shell_command(act))
			widget_fontbutton_input_by_command(var, act + 8);
		...
		if (strncasecmp(act, "file:", 5) == 0 && strlen(act) > 5) {
			...
			widget_fontbutton_input_by_file(var, act + 5);
		}
	...
	if (attributeset_is_avail(var->Attributes, ATTR_ITEM))
		widget_fontbutton_input_by_items(var);
	...
	if (!initialised) {
		...
		/* Connect signals */
		g_signal_connect(G_OBJECT(var->Widget), "font-set",
			G_CALLBACK(on_any_widget_font_set_event), (gpointer)var->Attributes);
...
static void widget_fontbutton_input_by_command(variable *var, char *command)
{
	...
	widget_opencommand2(command, line, line_size);
	if (line[0] != 0){
