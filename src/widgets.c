...
extern gboolean option_nonblocking; // wiak added 29May2018
extern gboolean option_blocking;    // wiak added 29May2018
extern gchar *getenv_ptr; // wiak added
/***********************************************************************
 *                                                                     *
 ***********************************************************************/
 ...
 *                                                                     *
 ***********************************************************************/
/* wiak: legacy widget_opencommand function below still used by widgets
 * _list, _table, _tree, _terminal, _text, _comboboxtext.c, and 
 * _progressbar until at least until such time as I study their
 * functional details and find way to use alternative
 * glib sync/async execs. Current modded default
 * is to use legacy popen with /bin/sh unless env GTKWMODE=B */
...
FILE *widget_opencommand(const char *command)
{
	...
	if (option_include_file != NULL) {
		...
		if (*getenv_ptr == 'S')
			...
			infile = popen_bash(the_line, "r");		
		g_free(the_line);
	} else {
		if (*getenv_ptr == 'S')
			infile = popen(command, "r");
		else
			infile = popen_bash(command, "r");
	}
 ...
 *                                                                     *
 ***********************************************************************/
/* wiak: added this new function to select input command entry mode for
 * all widgets except _list, _table, _tree, _terminal, _text, and _progressbar.
 * i.e. It is used by widgets _button.c, _checkbox.c, _colorbutton.c,
 * _entry.c, _expander.c, _fontbutton.c, _frame.c,
 * _hscale.c, _menuitem.c, _notebook.c, _radiobutton.c, _spinbutton.c,
 * _statusbar.c, _time.c, _window.c and signals.c */

void widget_opencommand2(const char *command, gchar *line, size_t line_size)
{
	gchar *the_line, *stdout_line;
	FILE *infile;
	gint count;
	...
	if (*command == '-')
		switch(*(command+1)) {
		case 'a': // use sync glib blocking rather than async
		case 'b': // use sync glib blocking
			g_spawn_command_line_sync(the_line, &stdout_line, NULL, NULL, NULL);
			g_strlcpy(line, stdout_line, line_size); // guarantees null-termination. wiak: add line_size
			break;
		case 'B': // system_bash
			infile = popen_bash(the_line, "r");
			if (infile == NULL) 
	...
	else if (option_blocking || option_nonblocking) { // use sync glib blocking
		g_spawn_command_line_sync(the_line, &stdout_line, NULL, NULL, NULL);
		g_strlcpy(line, stdout_line, line_size);
	} else {
		if (*getenv_ptr == 'S') {
			infile = popen(the_line, "r");
			if (infile == NULL) 
		...
		else if ((*getenv_ptr == 'b') || (*getenv_ptr == 'a')) {  // use sync glib blocking
			g_spawn_command_line_sync(the_line, &stdout_line, NULL, NULL, NULL);
			g_strlcpy(line, stdout_line, line_size);
			...
			if (fgets(line, line_size, infile))
				line[line_size - 1] = 0;
			pclose(infile); // added here since no longer done in the widget_caller
		}
	}
	if (option_include_file != NULL)
		g_free(the_line);
